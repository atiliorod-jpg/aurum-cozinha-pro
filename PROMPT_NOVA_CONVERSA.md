...
### Rodada 18/07/2026 (parte 2) — feedback bug, textos, Novidades, Pix pagador, relatórios por destino
- **Bug do feedback corrigido:** o modal é filho do cabeçalho (texto branco) → os campos herdavam cor branca (texto invisível no branco). Fix: `text-gray-900` nos campos + no container.
- **Header:** rótulos pequenos sob os ícones (Ajuda/Histórico/Sair); nome do usuário no botão; `nomeExibicao` = nome OU "Administrador" (super-admin sem perfil) → fim do "Encerrar a sessão de null".
- **Textos "polo" removidos** de tudo que é visível: presetPolo locais → "Unidade Centro/Praia", demo, Config placeholder, min/máx, Login, GuideTour, Registrar, sugestoes. "polo-navy/gold" (cor) mantidos. Descrição de Entrada avulsa → "Item só porcionado, sem receita (ex.: picanha cortada)".
- **Novidades (changelog):** `data/novidades.js` (APP_VERSAO + NOVIDADES), página `/novidades`, popup `NovidadesPopup` que aparece 1x quando `APP_VERSAO` ≠ `localStorage aurum_versao_vista`; link em Config→Sistema. Para anunciar: editar novidades.js (item no topo) + bumpar APP_VERSAO.
- **Pix pagador:** "Já paguei" agora revela campo do NOME de quem pagou → `avisarPagamento(plano, nome)`; **migration14** (coluna `aviso_pagamento_nome` + RPC com `p_nome`, `ativar/limpar` zeram o nome) — **AINDA PENDENTE** (dashboard travou; app tem fallback pro RPC antigo). Admin mostra "avisou pagamento — plano X por NOME em dd/mm hh:mm" (`dataBRHora`). Orientação na tela: valor já vem no QR.
- **Relatórios básicos** (`utils/relatorios.js`, testado): `saidasPorDestinoDia` (por destino, por dia, ignora produção, não mistura) + `chegadasPorDia` (peso kg/dia). Duas seções novas no fim do Relatório. Resto (quinzenal/mensal por item, rendimento x aparas x desperdício) desenhado pro futuro.
- 66 testes, lint 0, build ok, smoke no preview (feedback dark, header, novidades, relatório) sem erros.
- **Futuro (semiacabados na cozinha de finalização):** tratar a cozinha de finalização como 2º estoque que RECEBE as saídas do porcionamento como entradas; o histórico por destino é a ponte.

### Rodada 19/07/2026 — auditoria/correção FINAL (A1–A3, B2, demo porção, rate limit)
- **A1 docs:** apagada a migration14 DUPLICATA (`migration14_pagador.sql`); README arrumado (numeração, status, query de checagem `conv_ins_v11`); removido o login de teste morto (`teste-prod`).
- **A2 rate limit (migration16):** `avisar_pagamento` recusa 2º aviso < 1h; `enviar_feedback` máx 10/h por usuário. **Bug achado e corrigido:** a m13/m14 tinham criado DUAS `avisar_pagamento` (1 arg sem freio + 2 args) — `create or replace` vira sobrecarga, não substitui; dropei a de 1 arg (senão furava o rate limit). Client já mostra o erro via toast.
- **A3 webhook honesto:** webhook limpa `aviso_pagamento_*` ao ativar; comentário "+31 só mensal"; STRIPE_SETUP com aviso "hoje a UI é Pix, Stripe é opcional".
- **B2:** toggle mostrar senha no NovaSenha (Login já tinha); **`PERMISSOES_PADRAO.cozinha.removerRegistros` = false** (só gerência+ apaga por padrão; diretoria libera na matriz).
- **Demo porção-centrada:** produtos em porção/unidade (Picanha/Frango/Tilápia/Empanado em unid), matéria-prima crua e granel em kg (Filé cru, Queijo, Batata). Recorte "entrada avulsa de porcionado" e produção coerentes.
- **Verificação do Supabase completa** (SQL): policies convites v11, perfis sem INSERT, todas as RPCs presentes, `notas_admin` removida, colunas comerciais ok. Pentest re-rodado 13/13 + m11. **12 contas `pentest.*` que tinham sobrado foram apagadas**; novo `scripts/pentest-limpar.mjs`. 68 testes, lint 0.
- **Auditoria externa 19/07 conferida:** era precisa; a maioria dos apontamentos era real (docs/migration14, rate limit, webhook dormente). Aplicados A1/A2/A3/B2. Backlog consciente mantido (Sentry, e-mail confirmação, produção atômica).

### Rodada 18/07/2026 (parte 4) — Relatórios completos + feedback no admin + doc multi-cozinha
- **Relatórios reorganizados** (`Relatorio.jsx` + `utils/relatorios.js`): seletor ÚNICO de período (Hoje/7d/Quinzena/Mês/Personalizado) no lugar das 2 abas; nova visão **Rendimento por item** (`rendimentoPorItem`: chegou kg → aparas → perdas → rendimento% via compraId) + **Produção por item** (`producaoPorItem`); **Excel REMOVIDO do relatório** (dono achou desnecessário) e trocado por **impressão/PDF bonito** (cabeçalho só-print + break-inside avoid no `index.css`). xlsx segue em Configuracoes (planilha modelo).
- **Feedback vai pro super-admin** (não mais WhatsApp): `migration15` tabela `feedback` (sem policy client) + RPC `enviar_feedback` (SECURITY DEFINER, vale vencido) + `feedback_todos`/`marcar_feedback` (super-admin). `BotaoFeedback` salva no banco (na demo = simulado); seção **📨 Feedback dos clientes** no /admin com marcar resolvido/reabrir. Bug do texto branco-no-branco já estava corrigido (`text-gray-900`).
- **migration14** (nome de quem pagou o Pix) formalizada em arquivo .sql.
- **Doc de estruturação na Área de Trabalho** (`OneDrive/Área de Trabalho/PROMPT_AURUM_MULTICOZINHA_2026-07.txt`): visão futura do dono — seletor de cozinha ao abrir (Produção/Finalização/Estoque Seco), Finalização (recebe auto da Produção, conta sobra no fim do turno, 2 turnos, fichas técnicas, desperdício), Estoque Seco clássico; decisão A (mesma conta multi-local) × B (contas separadas); fatiamento em 3 fases. **NÃO construído — só desenho.**
- E-mail de confirmação: **ADIADO** por decisão do dono. 68 testes, lint 0, build ok.
